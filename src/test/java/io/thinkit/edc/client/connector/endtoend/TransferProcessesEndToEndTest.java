...
import io.thinkit.edc.client.connector.services.management.*;
...
class TransferProcessesEndToEndTest extends RealTimeConnectorApiTestBase {
    ...
    TransferProcessesEndToEndTest(String managementVersion) {
    ...
    void should_get_a_transfer_process() {
        ...
        var contractAgreementId = negotiation.getContent().contractAgreementId();

        var transferProcessId = consumerClient.transferProcesses().create(transferProcessRequest(contractAgreementId));

        assertThat(transferProcessId.isSucceeded()).isTrue();
        assertThat(transferProcessId.getContent()).isNotNull();
    ...
    private TransferRequest transferProcessRequest(String contractAgreementId) {
        var dataDestination = Map.of("type", "HttpProxy");
